...
def stamp_args_file(
        *,
        ...
        values = [],
        image_pushers = []):
    """Generate the runner args file for a helm install/command action.

    Args:
        ctx (ctx): The rule context.
        helm_toolchain (ToolchainInfo): The resolved `@rules_helm//helm:toolchain_type` toolchain info.
        raw_args (Args): An `Args` object containing the raw arguments to forward to the runner.
        output (File): The file to write the generated arguments file to.
        chart (File, optional): Packaged chart file to include via `-chart`.
        values (list of File, optional): Values files to include via repeated `-values`.
        image_pushers (list of File, optional): Image pusher executables to include via `-image_pushers`.

    Returns:
        File: The `output` file (returned for convenience).
    """
    inputs = []
...
def expand_opts(ctx, opts, targets):
...
def _helm_install_impl(ctx, subcommand = "install"):
    ...
    args.add(subcommand)
    args.add_all(expand_opts(ctx, ctx.attr.opts, ctx.attr.data))
    args.add(install_name)
    args.add(rlocationpath(pkg_info.chart, ctx.workspace_name))

    args_file = stamp_args_file(
        ctx = ctx,
