...
func installHelmContent(workingDir string, packagePath string, stampedChartContent string, stampedValuesContent string, stampedSchemaContent string, templatesManifest string, filesManifest string, crdsManifest string, depsManifest string) (string, error) {
	...
	for crdPath, crdShortpath := range crds {
		...
		if fileInfo.IsDir() {
			...
			// Locate the crds directory so we can start copying files
			// into the new crds directory at the right location. If the
			// shortpath has no `crds/` ancestor (e.g. CRDs generated by
			// `controller-gen` and consumed from a sibling package), fall
			// back to using the file's parent directory as the root so the
			// CRD is staged flat as `crds/<basename>`. Without this
			// fallback, `filepath.Dir(".")` returns `"."` and the loop
			// spins indefinitely.
			if len(crdsRoot) == 0 {
				var current = filepath.Clean(crdShortpath)
				for {
					if len(current) == 0 || current == "." {
						crdsRoot = filepath.Dir(filepath.Clean(crdShortpath))
						break
					}
					parent := filepath.Dir(current)
			...
			if crdsRoot != "." && !strings.HasPrefix(filepath.Clean(crdShortpath), crdsRoot) {
				return "", fmt.Errorf(
